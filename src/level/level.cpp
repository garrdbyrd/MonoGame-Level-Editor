...
bool Level::readFromFile(const std::string &filename)
{
    ...
    if (!readMagicNumber(file)) {
        std::cerr << "Invalid file format (magic number mismatch)."
                  << std::endl;
        return false;
    }
...
bool Level::readMagicNumber(std::ifstream &file)
{
    ...
    return std::memcmp(fileMagicNumber, magicNumber, sizeof(fileMagicNumber)) ==
           0;
}
...
bool Level::readLevelTitle(std::ifstream &file)
{
    ...
    if (!file) {
        std::cerr << "Failed to seek to the levelTitle position in the "
                     "file."
                  << std::endl;
        return false;
    }
